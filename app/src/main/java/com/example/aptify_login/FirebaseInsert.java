...
public class FirebaseInsert {
        ...
        login_ref.child(uid).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if(task.isSuccessful()){
                    ...
                    if(signUpData!=null) {
                        username = signUpData.getFullname();
    ...
    public boolean insert(String game_name, Score score, Context context){
        ...
        DbRef.child(uid).child(date_time).setValue(score).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {

                if(task.isSuccessful()){
                    LeaderBoard_score ld_score= new LeaderBoard_score(username, score.getScore());
